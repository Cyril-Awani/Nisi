...
export default function YearlyPlansModal({ isOpen, onClose }: YearlyPlansModalProps) {
  ...
  const calculatePrice = (): number => {
    ...
    return Math.round(selectedPlan.price * selectedDuration.months * discountMultiplier);
  };
    ...
    >
      <div className="w-full max-w-md rounded-lg bg-white shadow-xl overflow-hidden animate-fade-in">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                const duration = durations.find(d => d.months === Number(e.target.value));
            ...
            {selectedDuration.months > 1 && selectedPlan.hasDiscount && (
              <p className="text-sm text-green-600 mt-2">
                You save {formatPrice(
                  selectedPlan.price * selectedDuration.months - calculatePrice()
        ...
        <div className="bg-gray-50 px-6 py-4 flex justify-end space-x-3">
          ...
          <button
            onClick={() => {
              alert(`Subscribed to ${selectedPlan.name} plan for ${selectedDuration.months} months!`);
              onClose();
            }}
